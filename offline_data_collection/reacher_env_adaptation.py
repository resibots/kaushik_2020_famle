    ...
    def cost_fn(self, samples):
        ...
        for i in range(n_batch): 
            ...
            for h in range(self.__horizon):
                ...
                eff_cost = -torch.exp(-torch.pow(start_states[:,-2::] - self.__goal,2).sum(dim=1))
                all_costs[start_index: end_index] +=  eff_cost * config["discount"]**h
...
def execute(env, init_state, steps, init_mean, init_var, model, config, last_action_seq, pred_high, pred_low):
    ...
    bar = ProgBar(steps, track_time=True, title='\nExecuting....', bar_char='▒')
    for i in range(steps):
        cost_object = Cost(ensemble_model=model, init_state=current_state, horizon=config["horizon"], action_dim=env.action_space.shape[0], goal=config["goal"], pred_high=pred_high, pred_low=pred_low) 
        config["cost_fn"] = cost_object.cost_fn   
        optimizer = RS_opt(config)
        sol = optimizer.obtain_solution()             
        ## Take soft action        
        a = sol[0:env.action_space.shape[0]] #if i == 0 else sol[0:env.action_space.shape[0]] * 0.8 + a * 0.2
        next_state, r = 0, 0 
        for k in range(1):
            next_state, r, _, _ = env.step(a)
        trajectory.append([current_state.copy(), a.copy(), next_state-current_state, -r])
...
config = {
    ...
    "record_video": False,
    "result_dir": "results",
    "env_name": "ensemble_reacher",
    "exp_suffix": "experiment",
    "exp_details": "Reacher default",
    "dump_trajects": 100,
    "data_dir": "",
    ...
    "cuda": True,
    "ensemble_epoch": 5,
    "ensemble_dim_in": 5+12,
    "ensemble_dim_out": 12,
    "ensemble_hidden": [50, 50],
